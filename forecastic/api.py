...
def _process_predictions(predictions: list[dict[str, Any]]) -> list[PredictionRow]:
    ...
    explanations = data[[c for c in data.columns if c.startswith("EXPLANATION_")]]

    clean_predictions = pd.concat([slim_predictions, intervals, explanations], axis=1)
...
def get_llm_summary(predictions: List[dict[str, Any]]) -> ForecastSummary:
    ...
    #exclude_target_summary = _summarize_dataframe(preds, ex_target=True)
...
def _summarize_dataframe(prompt_dataframe: pd.DataFrame, ex_target: bool) -> str:
    ...
    if ex_target:
        prompt = gettext(
            "The following are the most important external factors "
            + "in the forecasting model's predictions of `{target}`. "
            + "Provide a 3-4 sentence summary of the external"
            + "driver(s) for the forecast, explain any potential "
            + "intuitive, qualitative interpretation(s) "
            + "or explanation(s)."
            + "Answer in a visually clear manner using a list format"
            + "Answer in Japanese."
        ...
        prompt = gettext(
            "The following are the most important factors in the "
...
def _get_prompt(
    ...
    top_features_string = prediction_explanations_df.to_string()
...
def _make_headline(standardized_predictions: list[PredictionRow]) -> str:
    ...
    return _get_completion(
        prompt=gettext("Forecast:") + str(df[["date_id", "prediction"]]) + gettext("Forecast's lower and upper limit:") + str(df[['low', 'high']]) + gettext("Rationale for the forecast:") + str(df[[c for c in df.columns if c.startswith("EXPLANATION_")]]),
